...
namespace ReplayHandler.Classes
{
    internal class HQMParse
    {
        ...
        public static Tuple<uint, uint> ConvertMatrixToNetwork(int b, Matrix<double> v)
        {
            var r1 = ConvertRotColumnToNetwork(b, v.Column(1));
            var r2 = ConvertRotColumnToNetwork(b, v.Column(2));
            return Tuple.Create(r1, r2);
        }

        public static uint ConvertRotColumnToNetwork(int b, MathNet.Numerics.LinearAlgebra.Vector<double> v)
        {
            int res = 0;
            if (v[0] < 0.0)
            {
            ...
            var temp1 = TABLE[res][0];
            var temp2 = TABLE[res][1];
            var temp3 = TABLE[res][2];

            for (int i = 3; i < b; i += 2)
            {
                var temp4 = NormalizeVector((DLA.Vector)temp1.Add(temp2));
                var temp5 = NormalizeVector((DLA.Vector)temp2.Add(temp3));
                var temp6 = NormalizeVector((DLA.Vector)temp1.Add(temp3));

                var a1 = Cross((temp4 - temp6), (v - temp6));

                if (a1.DotProduct(v) < 0.0)
                {
                    var a2 = Cross((temp5 - temp4), (v - temp4));
                    if (a2.DotProduct(v) < 0.0)
                    {
                        var a3 = Cross((temp6 - temp5), (v - temp5));
                        if (a3.DotProduct(v) < 0.0)
                        {
                            res |= 3 << i;
                            temp1 = (DLA.Vector)temp4;
                            temp2 = (DLA.Vector)temp5;
                            temp3 = (DLA.Vector)temp6;
                        }
                        else
                        {
                            res |= 2 << i;
                            temp1 = (DLA.Vector)temp6;
                            temp2 = (DLA.Vector)temp5;
                        }
                    }
                    else
                    {
                        res |= 1 << i;
                        temp1 = (DLA.Vector)temp4;
                        temp3 = (DLA.Vector)temp5;
                    }
                }
                else
                {
                    temp2 = (DLA.Vector)temp4;
                    temp3 = (DLA.Vector)temp6;
                }
            }
            return (uint)res;
