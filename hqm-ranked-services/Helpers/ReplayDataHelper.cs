...
namespace hqm_ranked_backend.Helpers
{
    public static class ReplayDataHelper
    {
        ...
        public static ReplayCalculatedData GetReplayCalcData(List<ReplayTick> ticks)
        {
            ...
            {
                Name= x.Message.PlayerName,
                Text = x.Message.Message,
                Packet = x.Packet
            }).ToList();
            ...
            j = 0;
            foreach (var tick in withVectors.Where((x, index)=> index %25 == 0))
            {
                    ...
                    if (puck.TouchedBy != null && shotDetected != null)
                    {
                        if (puck.TouchedBy != prevTouched)
                        {
                            if (foundPlayer != null)
                            {
                                ...
                                {
                                    var goalFound = result.Goals.Any(x => x.Packet > tick.Packet && x.Packet < tick.Packet + 300);
        ...
        public static ReplayCalculatedData GetReplayCalcDataNew(List<ReplayTick> ticks)
        {
            var result = new ReplayCalculatedData();
            ...
            var index = 0;
            j = 0;
            foreach (var tick in withVectors.Where(tick=> tick.Objects.Where(x => x.Type == ObjectType.Puck).Count() == 1))
            {
                var isPaused = result.Pauses.Any(x => x.StartPacket < tick.Packet && x.EndPacket > tick.Packet);
                if (isPaused)
                {
